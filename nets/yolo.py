...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth = 1.0, width = 1.0, in_features = ("dark2","dark3", "dark4", "dark5"), in_channels = [128, 256, 512, 1024], depthwise = False, act = "silu"):
        ...
        #-------------------------------------------#
        #   160, 160, 256 -> 160, 160, 128   新加的  PAN中的CSP
        #-------------------------------------------#
        self.C3_p1 = CSPLayer(
    ...
    def forward(self, input):
        ...
        #-------------------------------------------#
        # feat1 = self.att_3(feat1)
        P4_upsample = torch.cat([P4_upsample, feat1], 1)
        #-------------------------------------------#


        #------------------新加的部分--------------------------#
        P4_upsample = self.C3_p2(P4_upsample)

        P2          = self.reduce_conv2(P4_upsample)

        P2_upsample = self.upsample(P2)

        # feat0 = self.att_4(feat0)
        P2_upsample = torch.cat([P2_upsample, feat0], 1)
        #-------------------------------------------#
        #   160, 160, 256 -> 160, 160, 128
        #-------------------------------------------#
        # P2_upsample = self.att_7(P2_upsample)
        P2_out = self.C3_p1(P2_upsample)
